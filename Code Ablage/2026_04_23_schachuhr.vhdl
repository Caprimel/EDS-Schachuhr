...
Sekunde_counter: process(Clk,start_stop,Reset,player_select,Seconds_clk,Seconds_clk_last,player_last,input_select,min_plus, min_minus, sek_plus, sek_minus)
begin
--rest
if (rising_edge(Clk)) then
        ...
        if input_select ='0' then
            if player_select = '0' then -- player 0
                if min_plus = '1' then
                    ...
                    Reg_0 <= (Reg_0 + 60);
                elsif min_minus = '1' then
                    reg0_const<= reg0_const-60;
                    Reg_0 <= (Reg_0 - 60);
                elsif sek_plus = '1' then
                    reg0_const<= reg0_const+1;
                    Reg_0 <= (Reg_0 + 1);
                elsif sek_minus = '1' then
                    reg0_const<= reg0_const-1;
                    Reg_0 <= (Reg_0 - 1);
                else NULL;
                end if;
